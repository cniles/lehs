...
(defn decode-pct-encoded [s]
  ...
  (letfn [(recur-decode [s]
           (if (>= (count s) 3)
             (let [a (nth s 0)
                   b (nth s 1)
                   c (nth s 2)]
               (if (re-matches #"%[0-9A-Za-z][0-9A-Za-z]" (str a b c))
                 (cons (pct-to-char b c) (recur-decode (nthrest s 3)))
                 (cons a (recur-decode (rest s))))) s))]
    (apply str (recur-decode s))))

(defn plus-to-space [v]
  (replace {\+ \space} v))
...
(defn decode-url-encoded [s]
  (into {} (map #(to-2nd decode-pct-encoded (to-2nd plus-to-space (get-key-value % "=")))
